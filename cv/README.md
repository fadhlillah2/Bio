...
| [`resume-1pager-v1.3.txt`](resume-1pager-v1.3.txt) / `.pdf` | **Recruiter/ATS edition** — curated 1-page resume (v8.2 lineage), NOT the LinkedIn mirror. Full keyword SKILLS block, user-confirmed facts (JUnit 5/Mockito, 5-person team scope, "alongside Assist.id"), open-to line, searchable-as aliases. v1.3 (2026-07-12): availability line → "Open to full-time, remote (international) & consulting roles" (aligns with the site's canonical availability; dropped redundant "Central Jakarta" already in the contact line). v1.2 (2026-07-11, archived): open-to line "Central Jakarta, WIB" → "Central Jakarta, WIB · UTC+7"; ATS-hardened template (linear headers, name extracts as the single token `FADHLILLAH`). v1.1 (2026-07-10, archived): "mentor 5 engineers" → "mentor peers", locations to SoT precision, Meta Llama casing, Hugging Face spelling. |
| [`consulting-onepager-v1.3.txt`](consulting-onepager-v1.3.txt) / `.pdf` | **Consulting one-pager (EN)** for CEOs/CTOs/business owners buying software development — outcomes in business language, services, verifiable proof links, engagement process. Linked from the site's Services section. v1.3 (2026-07-12): availability → "Open to full-time, remote (international), and consulting engagements" (aligns with site). v1.2 (2026-07-11, archived): headline "(WIB · UTC+7)" + page-fill layout (~90% of the A4). v1.1 (2026-07-10, archived): "7x" → sourced "700%", project name unified to High-Precision Contract-Advisor RAG, Llama casing, grammar fix. |
| [`consulting-onepager-id-v1.3.txt`](consulting-onepager-id-v1.3.txt) / `.pdf` | **Consulting one-pager (Bahasa Indonesia)** — faithful translation of the EN one-pager (same claims; numbers/URLs/nouns verbatim). v1.3 (2026-07-12, lock-step with EN): availability → "Terbuka untuk peran full-time, remote (internasional), dan konsultasi." v1.2 (2026-07-11, archived): "(WIB · UTC+7)" + page-fill layout. v1.1 (2026-07-10, archived): lock-step with EN + "konsultasi" consistency. |
...
```
resume:      v7.1 ──► v8.0 ──► v8.1 ──► v8.2 (curated 1-page) ──► v8.3 (full LinkedIn mirror, 2 pages) ──► v8.4 (+ expert-advisor-mt-5 block; LinkedIn paste pending)
1pager:      resume-1pager v1.0 ──► v1.1 ──► v1.2 ──► v1.3 (recruiter/ATS edition, v8.2 lineage — parallel artifact, not a mirror; v1.3 = availability "full-time / remote (international) / consulting")
consulting:  consulting-onepager v1.0 ──► v1.1 ──► v1.2 ──► v1.3 (EN) ⇄ v1.0 ──► v1.1 ──► v1.2 ──► v1.3 (ID)   [v1.3 = availability "full-time / remote (international) / consulting"]
...
- On version bump, also update ALL version-pinned links:
  ...
  - `cv/resume-1pager-vX.Y.pdf` (recruiter/ATS 1-pager — bumps on its OWN `v1.x` scheme, not with
    resume-v8.4) — hero **primary** "Download CV" + contact block + Resume section;
...
- **Artifact roles:** `resume-v8.4` = official CV (LinkedIn mirror; v8.4 adds expert-advisor-mt-5, paste pending on LinkedIn);
