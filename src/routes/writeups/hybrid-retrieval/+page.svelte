...
<svelte:head>
  ...
  <meta name="description" content="Why pure vector search loses clause numbers in contract documents, and how the Contract-Advisor RAG combines semantic search, TF-IDF, and an exact-match fallback — with the parts that are still unfinished.">
  ...
  <meta property="og:description" content="Why pure vector search loses clause numbers in contract documents, and how the retrieval layer is put together — including what is still unfinished.">
  ...
  <meta name="twitter:description" content="Why pure vector search loses clause numbers in contract documents — and what is still unfinished.">
    ...
    <article class="article">
      <div class="wrap article-wrap">
        ...
        <header class="article-head" data-reveal>
          ...
          <p class="article-deck">A contract question like <em>“what does PASAL 5 say?”</em> breaks a RAG pipeline in two unrelated places — and neither of them is the language model. Here is how the retrieval layer in Contract-Advisor RAG is put together, and which parts of it I have not finished.</p>
        ...
        <p>The second failure is retrieval. Even with clean text, asking for a specific clause number is exactly the query type that embedding search is worst at. Embeddings encode meaning, and every clause in a contract means roughly the same thing to a sentence transformer: obligations between parties. The number is the whole query, and it is the part that gets averaged away. Ask for clause 5 and you reliably get clause 4, 6, and 12 — all excellent semantic matches, all wrong.</p>
        ...
        <p>After extraction, a normalisation pass repairs the specific damage OCR does to Indonesian legal structure — collapsing <code>PASAL␠␠</code> back to <code>PASAL␠</code>, and folding <code>Pasal</code> into <code>PASAL</code> so casing does not fragment the token. It is deliberately narrow: it fixes what the OCR reliably breaks, and does not attempt to parse the document into a clause tree.</p>
        ...
        <p>That fallback is the honest answer to “why not just use vector search”. Semantic retrieval is the right default for <em>“what happens if we terminate early?”</em> and structurally incapable of reliably answering <em>“what does clause 5 say?”</em>. Rather than making embeddings do a job they are wrong for, the exact path bypasses them entirely — but only for the queries that need it.</p>

        <h2>What is not finished</h2>

        <p>Three things, and I would rather name them than let a reader find them. The first is now fixed — I have left it in place with what changed, because the bug the fix uncovered is more instructive than the patch.</p>

        <p><strong>The exact-match fallback was hard-coded.</strong> <span class="tag-fixed">Fixed Aug 2026</span> The pattern extraction in <code>_exact_section_search</code> tested for <code>'IV.15' in query</code> — a debugging shortcut from the hackathon that never got generalised, so every other clause reference fell through to semantic search. References are now derived from a shared <code>SECTION_PATTERNS</code> list, which the three duplicated inline copies of that list also read from.</p>
...
r'\s+'.join(re.escape(p) for p in "Section II.3".split())  <span class="tmut">→</span> Section\s+II\.3</code></pre>
        ...
        <p><strong>The section patterns are the wrong alphabet.</strong> Those regexes match <code>Section IV.15</code> and <code>Article IV</code> — Roman-numeral conventions from English contracts. The OCR layer normalises <code>PASAL</code> and <code>AYAT</code>, but the retriever never learned to look for them, so the exact path does not currently fire for the Indonesian documents the OCR pipeline was built for. The two halves were developed against different sample documents and never met.</p>
